...
INSERT INTO cd_order_250914 ( SELECT * FROM  cd_order  WHERE order_id >= ( SELECT MAX(order_id) FROM cd_order_250914 )  and  create_time<UNIX_TIMESTAMP('2025-10-30 16:00:00') LIMIT 300000 );
# 第二 删除50w数据
...
select  count(*) from    cd_order   where   create_time<UNIX_TIMESTAMP('2025-10-30 16:00:00');
...
INSERT INTO   cd_moneylog_250914   ( SELECT * FROM  cd_moneylog  WHERE moneylog_id >= ( SELECT MAX(moneylog_id) FROM   cd_moneylog_250914   ) and  create_time<UNIX_TIMESTAMP('2025-10-30 16:00:00') LIMIT 500000 );
# 第二 删除50w数据
delete  from    cd_moneylog   where    moneylog_id<=( SELECT MAX(moneylog_id) FROM   cd_moneylog_250914   )  limit 100000  ;
# 第三 查看订单号   可以不看
select  count(*) from    cd_moneylog   where    moneylog_id<=( SELECT MAX(moneylog_id) FROM   cd_moneylog_250914   )  ;
select  count(*) from    cd_moneylog   where     create_time<UNIX_TIMESTAMP('2025-10-30 16:00:00');
...
select  id,  DATE_FORMAT(FROM_UNIXTIME(createtime), '%Y-%m-%d %H:%i:%s') AS formatted_time  from   cd_request_log_1024    order by id  desc limit 10;
select  id,  DATE_FORMAT(FROM_UNIXTIME(createtime), '%Y-%m-%d %H:%i:%s') AS formatted_time  from   cd_request_log    order by id  asc limit 10;

INSERT INTO   cd_request_log_1024   ( SELECT * FROM  cd_request_log  WHERE id > ( SELECT MAX(id) FROM   cd_request_log_1024   ) LIMIT 500000 );

delete  from    cd_request_log   where    id<=( SELECT MAX(id) FROM   cd_request_log_1024   )  limit 300000  ;

select  count(*)   from  cd_request_log_1024;
