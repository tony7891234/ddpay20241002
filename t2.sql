INSERT INTO baxi_20241003.cd_order_1104 ( SELECT * FROM baxi_20241010.cd_order   where  order_id> 84130450 LIMIT 500000);
-- 1.获取新表最大的 id
...
-- 4。执行插入
INSERT INTO baxi_20241003.cd_order_1104 ( SELECT * FROM baxi_20241010.cd_order  where  order_id>=166112534  LIMIT 1000000);

--  5 取出最大值 比如 1001
select  order_id, orderid,  DATE_FORMAT(FROM_UNIXTIME(create_time), '%Y-%m-%d %H:%i:%s') AS formatted_time  from baxi_20241003.cd_order_1104  order by order_id  desc limit 5;
-- 6。删除 小于5的数据
delete  from   baxi_20241010.cd_order   where   order_id<=167112533  limit 300000;
--  查询还有多少条数据


-- 查询
select  count(*) from   baxi_20241010.cd_order    where    order_id<=166112533  ;
select  count(*) from   baxi_20241003.cd_order_1104     ;
...
select  moneylog_id,  DATE_FORMAT(FROM_UNIXTIME(create_time), '%Y-%m-%d %H:%i:%s') AS formatted_time  from baxi_20241003.cd_moneylog_1104  order by moneylog_id  desc limit 5;
...
INSERT INTO baxi_20241003.cd_moneylog_1104 ( SELECT * FROM baxi_20241010.cd_moneylog  where  moneylog_id>=183339581   LIMIT 1000000);
select  moneylog_id,  DATE_FORMAT(FROM_UNIXTIME(create_time), '%Y-%m-%d %H:%i:%s') AS formatted_time  from baxi_20241003.cd_moneylog_1104  order by moneylog_id  desc limit 5;

select  count(*) from   baxi_20241003.cd_moneylog_1104  ;
select  count(*) from   baxi_20241010.cd_moneylog  where  moneylog_id<=  182339580;
...
select  count(*) from  baxi_20241010.cd_moneylog     where    moneylog_id<=177339580  ;
...
INSERT INTO  map_order.cd_map_order  (orderid,sf_id,`table_name`)  (SELECT  orderid,sf_id,'cd_moneylog_1104'  FROM  baxi_20241003.cd_order_1104   );
...
CREATE TABLE merchant_callbacks (
    ...
    status tinyint(4) NOT NULL DEFAULT '0' COMMENT '状态：0待处理 1成功 2失败',
    response text DEFAULT NULL COMMENT '回调响应',
    created_at timestamp NULL DEFAULT NULL,
    updated_at timestamp NULL DEFAULT NULL,
    PRIMARY KEY (id),
    KEY idx_merchant_status (merchant_id, status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='商户回调记录表';

CREATE TABLE merchant_configs (
    id bigint(20) unsigned NOT NULL AUTO_INCREMENT,
    merchant_id varchar(50) NOT NULL COMMENT '商户ID',
    sign_key varchar(255) NOT NULL COMMENT '签名密钥',
    callback_url varchar(255) NOT NULL COMMENT '回调地址',
    alert_email varchar(255) DEFAULT NULL COMMENT '告警邮箱',
    created_at timestamp NULL DEFAULT NULL,
    updated_at timestamp NULL DEFAULT NULL,
    PRIMARY KEY (id),
