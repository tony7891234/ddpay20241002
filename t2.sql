...
select  order_id, orderid,  DATE_FORMAT(FROM_UNIXTIME(create_time), '%Y-%m-%d %H:%i:%s') AS formatted_time  from baxi_20241003.cd_order_250315  order by order_id  desc limit 5;
...
-- 3.查询大表的总数据
select  count(*)  from  baxi_20241003.cd_order_250315 ;
-- 4。执行插入
INSERT INTO baxi_20241003.cd_order_250315 ( SELECT * FROM baxi_20241010.cd_order  where  order_id>=211312834  LIMIT 500000);
...
# 1.最大的 cd_order_250315.order_id   2. 这个id 之后的50w
# 第一 插入多少条数据
INSERT INTO baxi_20241003.cd_order_250315 ( SELECT * FROM baxi_20241010.cd_order  WHERE order_id >= ( SELECT MAX(order_id) FROM baxi_20241003.cd_order_250315 ) LIMIT 500000 );
# 第二 删除50w数据
delete  from   baxi_20241010.cd_order   where    order_id<=( SELECT MAX(order_id) FROM baxi_20241003.cd_order_250315 )  limit 500000  ;
# 第三 查看订单号   可以不看
select  count(*) from   baxi_20241010.cd_order   where    order_id<=( SELECT MAX(order_id) FROM baxi_20241003.cd_order_250315 )  ;
...
# 第一 插入多少条数据
INSERT INTO baxi_20241003.cd_moneylog_250315 ( SELECT * FROM baxi_20241010.cd_moneylog  WHERE moneylog_id >= ( SELECT MAX(moneylog_id) FROM baxi_20241003.cd_moneylog_250315 ) LIMIT 500000 );
# 第二 删除50w数据
delete  from   baxi_20241010.cd_moneylog   where    moneylog_id<=( SELECT MAX(moneylog_id) FROM baxi_20241003.cd_moneylog_250315 )  limit 500000  ;
# 第三 查看订单号   可以不看
select  count(*) from   baxi_20241010.cd_moneylog   where    moneylog_id<=( SELECT MAX(moneylog_id) FROM baxi_20241003.cd_moneylog_250315 )  ;
